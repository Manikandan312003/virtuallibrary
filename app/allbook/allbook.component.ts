import { Component } from '@angular/core';
import {APIService} from '../service/api.service'

import { Router } from '@angular/router';
import { ActivatedRoute } from '@angular/router';
import { HttpClient } from '@angular/common/http';


@Component({
  selector: 'app-allbook',
  templateUrl: './allbook.component.html',
  styleUrls: ['./allbook.component.css']
})
export class AllbookComponent {
  
  constructor(private api:APIService,private http:HttpClient,private route:Router,private acrouter:ActivatedRoute)
  {

  }


  images:any;
  rout(data:any){
    this.api.image=data;
    console.log(data)
    this.route.navigateByUrl("search")
  }
  
  // images = [  
  //   { img: "data:image/jpeg;base64," },  
  //   { img: "assets/images/book1.jpeg" },  
  //   { img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTZRIvBcUOQorAMuTwp36Xa0U95_PbDEgUQVQ&usqp=CAU" },  
  //   { img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT-MSHDeRRp2m86wMKYkqx88spBZvID-b4g2A&usqp=CAU" },  
  //   { img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR2SY1GWTfmceSCO_qzv3EXLwK04XLhlKZWvA&usqp=CAU" },  
  //   { img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQBUXSwyC2JhzM9vx9xE72Mmcy8IlOpLR1Crg&usqp=CAU" },  
  //   { img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQF4jN6kIAxYjwJYFdZfE3QPHnINMXm5EjExQ&usqp=CAU" },  
  //   { img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS8suuKmx86GJJ-gT53yeMechex8_ZdUstJZA&usqp=CAU" },  
  //   { img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS_C_5D_M36tkB0xZyvwEO12Lm48CBWNzxzRA&usqp=CAU"}];


    

    
    number=0;
    
    createRange(number:any){
      // return new Array(number);
      return new Array(number).fill(0)
        .map((n, index) => index + 1);
    }
    s:any[]=[];
    originalimages:any[]=[];
    check_dict:any;
    check:string=""

    ngOnInit(): void {

      this.acrouter.data.subscribe(res=>{this.check_dict=res
      console.log("HELLO",res["name"])})
      // this.api.getdata("\\getallbooks").subscribe(result=>{
      //   console.log(result)
      //   this.images=result
      // })
      this.check=""+this.check_dict["name"]
      console.log(this.check=="recent")
      console.log(this.check)
      console.log(typeof this.check)


      console.log(this.originalimages)
      if(this.check=="wish")
      {
        console.log("wish")
        this.http.post("http://127.0.0.1:5000//getwishbook/",JSON.stringify({"name":this.api.signed_user})).subscribe(result=>{
          console.log(result)
          this.images=result
        })
      }
      if(this.check=="recent"){
        console.log("recent+++")
        console.log({"name":this.api.signed_user})
        
      this.http.post("http://127.0.0.1:5000//getrecentbook/",JSON.stringify({"name":this.api.signed_user})).subscribe(result=>{
        console.log(result)
        this.images=result
      })
      }
      if(this.check=="book"){
        console.log("book")
        
      this.api.getdata("\\getallbooks").subscribe(result=>{
        this.images=result
        console.log("ALLBOOK",this.images)
      })
      }
    
}
}
