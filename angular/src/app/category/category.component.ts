import { Component } from '@angular/core';

import {APIService} from '../service/api.service'


@Component({
  selector: 'app-category',
  templateUrl: './category.component.html',
  styleUrls: ['./category.component.css']
})
export class CategoryComponent {
  constructor(private api:APIService)
  {

  }
  images:any;
  
  // images = [  
  //   { img: "data:image/jpeg;base64," },  
  //   { img: "assets/images/book1.jpeg" },  
  //   { img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTZRIvBcUOQorAMuTwp36Xa0U95_PbDEgUQVQ&usqp=CAU" },  
  //   { img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT-MSHDeRRp2m86wMKYkqx88spBZvID-b4g2A&usqp=CAU" },  
  //   { img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR2SY1GWTfmceSCO_qzv3EXLwK04XLhlKZWvA&usqp=CAU" },  
  //   { img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQBUXSwyC2JhzM9vx9xE72Mmcy8IlOpLR1Crg&usqp=CAU" },  
  //   { img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQF4jN6kIAxYjwJYFdZfE3QPHnINMXm5EjExQ&usqp=CAU" },  
  //   { img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS8suuKmx86GJJ-gT53yeMechex8_ZdUstJZA&usqp=CAU" },  
  //   { img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS_C_5D_M36tkB0xZyvwEO12Lm48CBWNzxzRA&usqp=CAU"}];


    

    
    number=0;
    
    createRange(number:any){
      // return new Array(number);
      return new Array(number).fill(0)
        .map((n, index) => index + 1);
    }
    s:any[]=[];
    originalimages:any[]=[];
    
    ngOnInit(): void {

      this.api.getdata("//getallbooks").subscribe(result=>{
        console.log(result)
        this.images=result
      })
      console.log(typeof this.images)
      console.log(this.images.slice(1,5))
      for (let c = 1; c < this.images.length-3; c*=3) {
                this.s=this.images.slice(c,c+4);
                console.log(this.images.slice(c,c+4))
                this.originalimages.push(this.s);
      }
      console.log(this.originalimages)
    
}
}